...
const columns = computed<DataTableColumns<ISupplier>>(() => [
  ...
  { title: t('common.category'), key: 'category', width: 150 },
  {
    ...
    key: 'rating',
    width: 130,
    render: (row) => h(NRate, { readonly: true, value: row.rating, size: 'small' }),
  },
  {
...
type NewSupplierDraft = Pick<
  ISupplier,
  'name' | 'contactName' | 'email' | 'phone' | 'category' | 'address'
>
...
<template>
  <div class="flex h-full flex-col gap-4">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{
            t('suppliers.summary', {
              ...
              active: store.activeSuppliersCount,
            })
          }}
        </p>
      </div>
      <NButton
        v-can="'erp.suppliers.create'"
        type="primary"
        class="min-h-11"
    ...
    <div class="flex items-center gap-2">
      <NInput
        :value="store.supplierFilters.search"
      ...
      <NBadge :value="store.activeSupplierFilterCount" :show="store.activeSupplierFilterCount > 0">
        <NButton
          ...
          :aria-label="t('suppliers.advancedFilters')"
    ...
    <div v-if="!isCompact" class="min-h-0 flex-1">
      <NDataTable
        :columns="columns"
        :data="store.paginatedSuppliers"
        :pagination="pagination"
        :row-key="(row: ISupplier) => row.id"
        remote
        flex-height
        class="h-full"
        :bordered="false"
    ...
    <div v-else class="min-h-0 flex-1 overflow-y-auto">
      ...
      <div v-else class="flex flex-col gap-3 pb-4">
        <NCard
          v-for="supplier in store.paginatedSuppliers"
          :key="supplier.id"
          size="small"
          :bordered="true"
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            <NTag :type="statusTagType(supplier.status)" size="small" round class="shrink-0">{{
            ...
            <NPopconfirm @positive-click="() => handleDelete(supplier)">
              <template #trigger>
                <NButton
                  ...
                  circle
                  class="min-h-11 min-w-11"
                  :aria-label="`${t('common.delete')} ${supplier.name}`"
                >
                  <template #icon
                    ><NIcon><Trash /></NIcon
                  ></template>
      ...
      <NPagination
        v-if="store.suppliersPageCount > 1"
        :page="store.supplierPage"
        :page-count="store.suppliersPageCount"
        simple
        class="mt-2 justify-center"
        @update:page="(p: number) => (store.supplierPage = p)"
      />
    </div>
    ...
    <NModal
      ...
      preset="card"
      :title="t('suppliers.modalTitle')"
      :style="{ width: isCompact ? '92%' : '480px' }"
    >
      <NForm ref="createFormRef" :model="newSupplier" :rules="createRules" label-placement="top">
        <NFormItem :label="t('suppliers.companyName')" path="name"
          ><NInput v-model:value="newSupplier.name"
        /></NFormItem>
        <NFormItem :label="t('suppliers.contactName')" path="contactName"
          ><NInput v-model:value="newSupplier.contactName"
        /></NFormItem>
        <div class="grid grid-cols-2 gap-3">
          <NFormItem :label="t('common.email')" path="email"
            ><NInput v-model:value="newSupplier.email"
          /></NFormItem>
          <NFormItem :label="t('common.phone')" path="phone"
            ><NInput v-model:value="newSupplier.phone"
          /></NFormItem>
        </div>
        <NFormItem :label="t('common.category')" path="category"
          ><NInput
            v-model:value="newSupplier.category"
            :placeholder="t('suppliers.categoryPlaceholder')"
        /></NFormItem>
        <NFormItem :label="t('suppliers.address')" path="address"
          ><NInput v-model:value="newSupplier.address"
        /></NFormItem>
      </NForm>
      <template #footer>
