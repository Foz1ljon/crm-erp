...
import { SHIPMENT_STATUSES, type IShipment, type ShipmentFilters, type ShipmentStatus } from '@/types/erp'
...
const statusOptions = computed<SelectOption[]>(() => SHIPMENT_STATUSES.map((s) => ({ label: statusLabel(s), value: s })))
const statusDropdownOptions = computed<DropdownOption[]>(() => SHIPMENT_STATUSES.map((s) => ({ label: statusLabel(s), key: s })))
...
function handleStatusChange(shipment: IShipment, status: ShipmentStatus) {
  ...
  feedback.message?.success(t('logistics.statusUpdatedToast', { shipment: shipment.shipmentNumber, status: statusLabel(status) }))
}
...
const columns = computed<DataTableColumns<IShipment>>(() => [
  {
    ...
    render: (row) =>
      h('div', { class: 'min-w-0' }, [
        ...
        h('span', { class: 'block truncate text-xs text-gray-500 dark:text-gray-400' }, row.carrier),
      ]),
  },
  { title: t('logistics.route'), key: 'route', width: 200, render: (row) => `${row.origin} → ${row.destination}` },
  {
    ...
    render: (row) =>
      h(
        NDropdown,
        { options: statusDropdownOptions.value, trigger: 'click', onSelect: (key: string) => handleStatusChange(row, key as ShipmentStatus) },
        { default: () => h(NTag, { type: statusTagType(row.status), size: 'small', round: true, style: { cursor: 'pointer' } }, { default: () => statusLabel(row.status) }) },
      ),
  },
  { title: t('logistics.eta'), key: 'estimatedArrival', width: 130, render: (row) => dateFormatter.format(new Date(row.estimatedArrival)) },
  { title: t('logistics.trackingNumber'), key: 'trackingNumber', width: 150 },
  {
    ...
    render: (row) =>
      h(
        ...
        {
          trigger: () =>
            h(NButton, { quaternary: true, circle: true, size: 'small', 'aria-label': `${t('common.delete')} ${row.shipmentNumber}` }, { icon: () => h(NIcon, null, { default: () => h(Trash) }) }),
  ...
  <div class="flex h-full flex-col gap-4">
    <div class="flex items-center justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('logistics.count', { count: store.shipmentsTotalCount }) }}</p>
      </div>
      <NButton v-can="'erp.logistics.create'" type="primary" class="min-h-11" @click="openCreateModal">
    ...
    <div class="flex items-center gap-2">
      <NInput :value="store.shipmentFilters.search" :placeholder="t('logistics.searchPlaceholder')" clearable class="min-h-11 flex-1" :aria-label="t('common.search')" @update:value="updateQuickSearch">
        <template #prefix><NIcon><Search /></NIcon></template>
      </NInput>
      <NBadge :value="store.activeShipmentFilterCount" :show="store.activeShipmentFilterCount > 0">
        <NButton class="min-h-11 min-w-11" :aria-label="t('logistics.advancedFilters')" @click="filterDrawerOpen = true">
    ...
    <div v-if="!isCompact" class="min-h-0 flex-1">
      <NDataTable :columns="columns" :data="store.paginatedShipments" :pagination="pagination" :row-key="(row: IShipment) => row.id" remote flex-height class="h-full" :bordered="false" />
    </div>

    <div v-else class="min-h-0 flex-1 overflow-y-auto">
      <div v-if="store.paginatedShipments.length === 0" class="py-12 text-center text-sm text-gray-500 dark:text-gray-400">{{ t('logistics.noMatch') }}</div>
      <div v-else class="flex flex-col gap-3 pb-4">
        <NCard v-for="shipment in store.paginatedShipments" :key="shipment.id" size="small" :bordered="true" content-style="padding: 14px;">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <p class="truncate text-sm font-semibold">{{ shipment.shipmentNumber }}</p>
              <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ shipment.carrier }}</p>
            </div>
            <NTag :type="statusTagType(shipment.status)" size="small" round class="shrink-0">{{ statusLabel(shipment.status) }}</NTag>
          </div>
          ...
          <div class="mt-1 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            ...
            <span>{{ t('logistics.eta') }} {{ dateFormatter.format(new Date(shipment.estimatedArrival)) }}</span>
          </div>
          <div class="mt-3 flex items-center justify-between gap-2 border-t border-surface-border pt-3 dark:border-surface-dark-border">
            ...
            <NPopconfirm @positive-click="() => handleDelete(shipment)">
              <template #trigger>
                <NButton quaternary circle class="min-h-11 min-w-11" :aria-label="`${t('common.delete')} ${shipment.shipmentNumber}`">
      ...
      <NPagination v-if="store.shipmentsPageCount > 1" :page="store.shipmentPage" :page-count="store.shipmentsPageCount" simple class="mt-2 justify-center" @update:page="(p: number) => (store.shipmentPage = p)" />
    </div>
      ...
      <NDrawerContent :title="t('logistics.advancedFilters')" closable>
        <div class="flex flex-col gap-5 pb-4">
          <NFormItem :label="t('common.search')" label-placement="top">
            <NInput v-model:value="draft.search" :placeholder="t('logistics.searchPlaceholder')" clearable />
          </NFormItem>
    ...
    <NModal v-model:show="createModalOpen" preset="card" :title="t('logistics.modalTitle')" :style="{ width: isCompact ? '92%' : '480px' }">
      <NForm ref="createFormRef" :model="newShipment" :rules="createRules" label-placement="top">
        <NFormItem :label="t('logistics.carrier')" path="carrier"><NInput v-model:value="newShipment.carrier" :placeholder="t('logistics.carrierPlaceholder')" /></NFormItem>
        <div class="grid grid-cols-2 gap-3">
          <NFormItem :label="t('logistics.origin')" path="origin"><NInput v-model:value="newShipment.origin" /></NFormItem>
          <NFormItem :label="t('logistics.destination')" path="destination"><NInput v-model:value="newShipment.destination" /></NFormItem>
        </div>
      </NForm>
      <template #footer>
        <div class="flex justify-end gap-3">
          <NButton class="min-h-11" @click="createModalOpen = false">{{ t('common.cancel') }}</NButton>
          <NButton class="min-h-11" type="primary" @click="submitCreate">{{ t('logistics.createShipment') }}</NButton>
        </div>
      </template>
    </NModal>
  </div>
