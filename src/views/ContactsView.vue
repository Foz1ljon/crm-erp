...
import { NButton, NIcon, NModal, NPopconfirm, NSelect, type DataTableColumns, type FormRules, type SelectOption } from 'naive-ui'
...
const customerOptions = computed<SelectOption[]>(() =>
  Array.from(new Map(store.contacts.map((c) => [c.customerId, c.customerName])).entries()).map(([value, label]) => ({ label, value })),
)
...
const columns = computed<DataTableColumns<IContact>>(() => [
  {
    ...
    render: (row) =>
      h('div', { class: 'flex min-w-0 items-center gap-2' }, [
        row.isPrimary ? h(NIcon, { class: 'shrink-0 text-brand-500' }, { default: () => h(Star) }) : null,
        h('div', { class: 'min-w-0' }, [
          h('span', { class: 'block truncate text-sm font-medium' }, row.name),
          h('span', { class: 'block truncate text-xs text-gray-500 dark:text-gray-400' }, row.role),
        ]),
      ]),
  },
  ...
  { title: t('contacts.lastContacted'), key: 'lastContactedAt', width: 140, render: (row) => dateFormatter.format(new Date(row.lastContactedAt)) },
  {
    title: '',
    key: 'actions',
    width: 56,
...
const allCustomerOptions = computed<SelectOption[]>(() => store.customers.map((c) => ({ label: c.name, value: c.id })))
  ...
  <div class="flex h-full flex-col gap-4">
    <div class="flex items-center justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('common.matching', { count: store.contactsTotalCount, item: t('contacts.title').toLowerCase() }) }}</p>
      </div>
      <NButton v-can="'crm.contacts.create'" type="primary" class="min-h-11" @click="openCreateModal">
    ...
    <div class="flex items-center gap-2">
      ...
      <NBadge :value="store.activeContactFilterCount" :show="store.activeContactFilterCount > 0">
        <NButton class="min-h-11 min-w-11" :aria-label="t('contacts.advancedFilters')" @click="filterDrawerOpen = true">
    ...
    <div v-if="!isCompact" class="min-h-0 flex-1">
      <NDataTable :columns="columns" :data="store.paginatedContacts" :pagination="pagination" :row-key="(row: IContact) => row.id" remote flex-height class="h-full" :bordered="false" />
    </div>

    <div v-else class="min-h-0 flex-1 overflow-y-auto">
      <div v-if="store.paginatedContacts.length === 0" class="py-12 text-center text-sm text-gray-500 dark:text-gray-400">{{ t('contacts.noMatch') }}</div>
      <div v-else class="flex flex-col gap-3 pb-4">
        <NCard v-for="contact in store.paginatedContacts" :key="contact.id" size="small" :bordered="true" content-style="padding: 14px;">
          <div class="flex items-start justify-between gap-3">
            <div class="flex min-w-0 items-center gap-2">
              ...
              <div class="min-w-0">
                ...
                <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ contact.role }} · {{ contact.customerName }}</p>
              </div>
            </div>
            <a :href="`tel:${contact.phone}`" class="shrink-0 rounded-full p-2 text-brand-600 dark:text-brand-400" :aria-label="t('contacts.callContact')"><NIcon><Phone /></NIcon></a>
          </div>
          ...
          <div class="mt-3 flex items-center justify-end gap-2 border-t border-surface-border pt-3 dark:border-surface-dark-border">
            <NPopconfirm @positive-click="() => handleDelete(contact)">
              <template #trigger>
                <NButton quaternary circle class="min-h-11 min-w-11" :aria-label="`${t('common.delete')} ${contact.name}`">
      ...
      <NPagination v-if="store.contactsPageCount > 1" :page="store.contactPage" :page-count="store.contactsPageCount" simple class="mt-2 justify-center" @update:page="store.setContactPage" />
    </div>
      ...
      <NDrawerContent :title="t('contacts.advancedFilters')" closable>
        <div class="flex flex-col gap-5 pb-4">
          <NFormItem :label="t('common.search')" label-placement="top">
            <NInput v-model:value="draft.search" :placeholder="t('contacts.searchPlaceholder')" clearable />
          </NFormItem>
          <NFormItem :label="t('contacts.filter.company')" label-placement="top">
            <NSelect v-model:value="draft.customerIds" :options="customerOptions" multiple :placeholder="t('common.any')" />
          </NFormItem>
    ...
    <NModal v-model:show="createModalOpen" preset="card" :title="t('contacts.modalTitle')" :style="{ width: isCompact ? '92%' : '480px' }">
      <NForm ref="createFormRef" :model="newContact" :rules="createRules" label-placement="top">
        <NFormItem :label="t('common.company')" path="customerId"><NSelect v-model:value="newContact.customerId" :options="allCustomerOptions" /></NFormItem>
        <NFormItem :label="t('common.name')" path="name"><NInput v-model:value="newContact.name" :placeholder="t('leads.namePlaceholder')" /></NFormItem>
        <NFormItem :label="t('contacts.role')" path="role"><NInput v-model:value="newContact.role" :placeholder="t('contacts.rolePlaceholder')" /></NFormItem>
        <NFormItem :label="t('common.email')" path="email"><NInput v-model:value="newContact.email" :placeholder="t('leads.emailPlaceholder')" /></NFormItem>
        <NFormItem :label="t('common.phone')" path="phone"><NInput v-model:value="newContact.phone" :placeholder="t('leads.phonePlaceholder')" /></NFormItem>
      </NForm>
      <template #footer>
        <div class="flex justify-end gap-3">
          <NButton class="min-h-11" @click="createModalOpen = false">{{ t('common.cancel') }}</NButton>
          <NButton class="min-h-11" type="primary" @click="submitCreate">{{ t('contacts.createContact') }}</NButton>
        </div>
      </template>
    </NModal>
  </div>
